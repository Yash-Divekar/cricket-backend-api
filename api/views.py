...
from .serializers import PlayerProfileSerializer, TeamSerializer, MatchSerializer
...
class PlayerView(APIView):
    ...
    def get(self, request, player_id=None):
        try:
            ...
            return paginator.get_paginated_response(api_response(data=serializer.data))
    ...
    def put(self, request, player_id):
        try:
            player = PlayerProfile.objects.get(id=player_id)
            if RoleEnum.PLAYER in request.user.roles and player.user != request.user:
